    ...
    , _runningUnitTests     (unitTesting)
{
    ...
    qDebug() << "APP_VERSION_STR : " << APP_VERSION_STR;
        ...
        if (versionFile.open(QIODevice::ReadOnly)) {
            ...
            if (_parseVersionText(version, majorVersion, minorVersion, buildVersion)) {
                if (_majorVersion < majorVersion ||
                    ...
                    showAppMessage(tr("Welcome to ROVOTUN Control Station").arg(applicationName()).arg(toolbox()->corePlugin()->stableDownloadLocation()), tr("Rovostech"));
                }
            }
        }
    } else {
        qDebug() << "stable version failed" << errorMsg;
    }
