...
Vehicle::Vehicle(LinkInterface*             link,
    ...
    ,_verval(0)
    ,_topoffsetval(0)
    ,_activetopoffsetval(0)
    ,_downoffsetval(0)
    ,_activedownoffsetval(0)
    ,_leftoffsetval(0)
    ,_activeleftoffsetval(0)
    ,_rightoffsetval(0)
    ,_activerightoffsetval(0)
    ,_activeyawval(0)
    ,_activecruiseval(0)
    ,_activebackwardval(0)
    ,_calibsetval(1)
    ,_activecalibval(0)
//RYY
...
void Vehicle::_handleAttitudeWorker(double rollRadians, double pitchRadians, double yawRadians)
{
    ...
    roll = qRadiansToDegrees(rollRadians);
    pitch = qRadiansToDegrees(pitchRadians);
    yaw = qRadiansToDegrees(yawRadians);
...
void Vehicle::_handleAttitudeQuaternion(mavlink_message_t& message)
{
    ...
    pitchRate()->setRawValue(qRadiansToDegrees(rates[1])); //RYY
...
void Vehicle::connectionudp(int value){

    QUdpSocket *socket = new QUdpSocket(this);
    if (!socket->bind(QHostAddress::Any, value)) {
        qDebug() << "Failed to bind to port" << value;
        delete socket;
    }
    qDebug() << "Bound to port" << value;
    // Connection for handling data reception
    QObject::connect(socket, &QUdpSocket::readyRead,this, [socket, this]() {
        while (socket->hasPendingDatagrams()) {
            ...
            QString type = dataList.takeFirst(); // Extract the type
            QString joinedValues = dataList.join(" "); // Join remaining values into a single string

            if (type == "HNAV") {
                QStringList fieldNames = {"pitchinsM", "rollinsM", "yawinsM", "latitudeinsM", "longitudeinsM", "depthinsM","speedinsM"};
                processData("HNAV", joinedValues, fieldNames);
            } else if (type == "P96") {
                QStringList fieldNames = {"topprofM", "rightprofM", "downprofM", "leftprofM"};
                processData("P96", joinedValues, fieldNames);
            } else if (type == "Alti") {
                QStringList fieldNames = {"position", "AltM", "ConfiM"};
                processData("Alti", joinedValues, fieldNames);
            } else {
                // Process data with unknown type
                qDebug() << "Received data with unknown type:" << type;
            }
...
QMap<QString, QString> Vehicle::splitString(const QStringList& fieldNames, const QString& input) {
    // Remove \r\n from the input string
    QString cleanedInput = input.trimmed().remove("\r\n");
    // Split the cleaned input string by space
    QStringList fields = cleanedInput.split(" ", Qt::SkipEmptyParts);
    // Create a map to store the split fields
    QMap<QString, QString> fieldMap;
    // Assign field names to the fields
    for (int i = 0; i < qMin(fields.size(), fieldNames.size()); ++i) {
        fieldMap[fieldNames[i]] = fields.value(i);
    }
    return fieldMap;
}


void Vehicle::processData(const QString& type, const QString& joinedValues, const QStringList& fieldNames) {
    // Add processing logic here
    // Split received data into fields using splitString function
    QMap<QString, QString> splitedData = splitString(fieldNames, joinedValues);
    // Access each field and process data as needed

    // Store the received data into corresponding member variables
    storeData(type, splitedData);
    //qDebug() << splitedData;
}

void Vehicle::storeData(const QString& type, const QMap<QString, QString>& splitedData) {
    // Store the received data into corresponding member variables
    if (type == "HNAV") {
        pitchinsM = splitedData.value("pitchinsM").toFloat();
        rollinsM = splitedData.value("rollinsM").toFloat();
        yawinsM = splitedData.value("yawinsM").toFloat();
        latitudeinsM = splitedData.value("latitudeinsM").toFloat();
        longitudeinsM = splitedData.value("longitudeinsM").toFloat();
        depthinsM = splitedData.value("depthinsM").toFloat();
        speedM = splitedData.value("speedinsM").toFloat();
    } else if (type == "P96") {
        topprofM = splitedData.value("topprofM").toFloat();
        rightprofM = splitedData.value("rightprofM").toFloat();
        downprofM = splitedData.value("downprofM").toFloat();
        leftprofM = splitedData.value("leftprofM").toFloat();
    } else if (type == "Alti") {
        QString position = splitedData.value("position");
        if (position == "T") {
            topAltM = splitedData.value("AltM").toFloat();
            topConfiM = splitedData.value("ConfiM").toInt();
        } else if (position == "R") {
            rightAltM = splitedData.value("AltM").toFloat();
            rightConfiM = splitedData.value("ConfiM").toInt();
        } else if (position == "B") {
            downAltM = splitedData.value("AltM").toFloat();
            downConfiM = splitedData.value("ConfiM").toInt();
        } else if (position == "L") {
            leftAltM = splitedData.value("AltM").toFloat();
            leftConfiM = splitedData.value("ConfiM").toInt();
        }
    } else {
        // Process data with unknown type
        qDebug() << "Received data with unknown type:" << type;
    }
    emit dataChanged(); // Emit signal to notify UI of data change

}

float Vehicle::pitchinsM1() const { return pitchinsM; }
float Vehicle::rollinsM1() const { return rollinsM; }
float Vehicle::yawinsM1() const { return yawinsM; }
float Vehicle::depthinsM1() const { return depthinsM; }
float Vehicle::latinsM1() const { return latitudeinsM; }
float Vehicle::longinsM1() const { return longitudeinsM; }

float Vehicle::topprofM1() const { return topprofM; }
float Vehicle::rightprofM1() const { return rightprofM; }
float Vehicle::downprofM1() const { return downprofM; }
float Vehicle::leftprofM1() const { return leftprofM; }

float Vehicle::topAltM1() const { return topAltM; }
float Vehicle::rightAltM1() const { return rightAltM; }
float Vehicle::downAltM1() const { return downAltM; }
float Vehicle::leftAltM1() const { return leftAltM; }

float Vehicle::topConfiM1() const { return topConfiM; }
float Vehicle::rightConfiM1() const { return rightConfiM; }
float Vehicle::downConfiM1() const { return downConfiM; }
float Vehicle::leftConfiM1() const { return leftConfiM; }

float Vehicle::speedM1() const { return speedM; }

// RYY function

void Vehicle::setKalmangval(float value)
{
    if (_kalmangval != value) {
        _kalmangval = value;
        emit kalmangvalChanged(value); // Emit signal when the value changes
    }
}

float Vehicle::kalmangval() const
{
    return _kalmangval;
}

void Vehicle::setnoiseval(float value)
{
    if (_noiseval != value) {
        _noiseval = value;
        emit noisevalChanged(value); // Emit signal when the value changes
    }
}

float Vehicle::noiseval() const
{
    return _noiseval;
}

void Vehicle::setsafetyval(float value)
{
    if (_safetyval != value) {
        _safetyval = value;
        emit safetyvalChanged(value); // Emit signal when the value changes
    }
}

float Vehicle::safetyval() const
{
    return _safetyval;
}

void Vehicle::setthresholdval(float value)
{
    if (_centerthreshold != value) {
        _centerthreshold = value;
        emit thresholdvalChanged(value); // Emit signal when the value changes
    }
}

float Vehicle::thresholdval() const
{
    return _centerthreshold;
}

void Vehicle::setactivecenter(float value)
{
    if (_activeval != value) {
        _activeval = value;
        emit activevalChanged(value); // Emit signal when the value changes
    }
}

float Vehicle::activecenterval() const
{
    return _activeval;
}


void Vehicle::setactiveyaw(float value)
{
    if (_activeyawval != value) {
        _activeyawval = value;
        emit activeyawvalChanged(value); // Emit signal when the value changes
    }
}

float Vehicle::activeyawval() const
{
    return _activeyawval;
}

void Vehicle::setaltimeter(float value){

    if(_altimeterval != value){
        _altimeterval = value;
        emit altimetervalChanged(value);
    }
}

float Vehicle::altimeterval() const
{
    return _altimeterval;

}

void Vehicle::setprofiling(float value){

    if(_profilingval != value){
        _profilingval = value;
        emit altimetervalChanged(value);
    }
}

float Vehicle::profilingval() const
{
    return _profilingval;

}



void Vehicle::sethorizontal(float value){

    if(_horval != value){
        _horval = value;
        emit horvalChanged(value);
    }
}



float Vehicle::horizontalval() const
{
    return _horval;

}

void Vehicle::setvertical(float value){

    if(_verval != value){
        _verval = value;
        emit vervalChanged(value);
    }
}



float Vehicle::verticalval() const
{
    return _verval;

}

void Vehicle::setINS(float value){

    if(_insval != value){
        _insval = value;
        emit insvalChanged(value);
    }
}



float Vehicle::INSval() const
{
    return _insval;
...
void Vehicle::topoffset(float value){

    if(_topoffsetval != value){
        _topoffsetval = value;
        emit topoffsetvalChanged(value);
    }
}

float Vehicle::topoffsetval() const
{
    return _topoffsetval;

}

void Vehicle::activetopoffset(float value){

    if(_activetopoffsetval != value){
        _activetopoffsetval = value;
        emit activetopoffsetvalChanged(value);
    }
}

float Vehicle::activetopoffsetval() const
{
    return _activetopoffsetval;

}


void Vehicle::downoffset(float value){

    if(_downoffsetval != value){
        _downoffsetval = value;
        emit downoffsetvalChanged(value);
    }
}

float Vehicle::downoffsetval() const
{
    return _downoffsetval;

}



void Vehicle::activedownoffset(float value){

    if(_activedownoffsetval != value){
        _activedownoffsetval = value;
        emit activedownoffsetvalChanged(value);
    }
}

float Vehicle::activedownoffsetval() const
{
    return _activedownoffsetval;
...
void Vehicle::leftoffset(float value){

    if(_leftoffsetval != value){
        _leftoffsetval = value;
        emit leftoffsetvalChanged(value);
    }
}

float Vehicle::leftoffsetval() const
{
    return _leftoffsetval;

}



void Vehicle::activeleftoffset(float value){

    if(_activeleftoffsetval != value){
        _activeleftoffsetval = value;
        emit activeleftoffsetvalChanged(value);
    }
}

float Vehicle::activeleftoffsetval() const
{
    return _activeleftoffsetval;
...
void Vehicle::rightoffset(float value){

    if(_rightoffsetval != value){
        _rightoffsetval = value;
        emit rightoffsetvalChanged(value);
    }
}

float Vehicle::rightoffsetval() const
{
    return _rightoffsetval;

}



void Vehicle::activerightoffset(float value){

    if(_activerightoffsetval != value){
        _activerightoffsetval = value;
        emit activerightoffsetvalChanged(value);
    }
}

float Vehicle::activerightoffsetval() const
{
    return _activerightoffsetval;

}

void Vehicle::activecruise(float value){

    if(_activecruiseval != value){
        _activecruiseval = value;
        emit activecruisevalChanged(value);
    }
}

float Vehicle::activecruiseval() const
{
    return _activecruiseval;

}

void Vehicle::activebackward(float value){

    if(_activebackwardval != value){
        _activebackwardval = value;
        emit activebackwardvalChanged(value);
    }
}

float Vehicle::activebackwardval() const
{
    return _activebackwardval;

}

void Vehicle::activecalib(float value){

    if(_activecalibval != value){
        _activecalibval = value;
        emit activecalibvalChanged(value);
    }
}

float Vehicle::activecalibval() const
{
    return _activecalibval;

}


void Vehicle::setcalibval(float value)
{
    if (_calibsetval != value) {
        _calibsetval = value;
        emit altitudevalChanged(value); // Emit signal when the value changes
    }
}

float Vehicle::calibval() const
{
    return _calibsetval;
}


void Vehicle::offsetkalman(int pilih, float roll_1, float pitch_1, float yaw_1, float thrust_1, quint16 buttons_1){
    //qDebug() << "offset activated";
    SharedLinkInterfacePtr sharedLink = vehicleLinkManager()->primaryLink().lock();
    if (!sharedLink) {
    ...
    float R = _noiseval; // Measurement noise covariance
    float K = _kalmangval; //kalman gain
    ...
    float CenterThreshold = _centerthreshold;// center threshold
    float x_roll = roll_1; // Initial roll estimate
    float x_thrust = thrust_1; // Initial thrust estimate
    float x_yaw = yaw_1; // Initial thrust estimate
    float pitch_cruise = pitch_1;
    ...
    if (_altimeterval == 1 && _profilingval == 1){
        topError = (topAltM+topprofM)/2 - _topoffsetval;
        botError = _downoffsetval - (downAltM+downprofM)/2 ;
        rightError = (rightAltM+rightprofM)/2 - _rightoffsetval;
        leftError = _leftoffsetval - (leftAltM+leftprofM)/2;
        error_yaw = (leftprofM-leftAltM) - (rightprofM-rightAltM)  ;
        N_yaw = P_yaw / (P_yaw + R);
        x_yaw = x_yaw + N_yaw * error_yaw * K;
        if (_activecruiseval == 1){
            pitch_cruise = pitch_cruise  + _safetyval *250;
            // backward
            if (_activebackwardval == 1){
                pitch_cruise = pitch_cruise * -1;
            }

        }

    } else if (_altimeterval == 1){
        topError = topAltM - _topoffsetval;
        botError = _downoffsetval - downAltM ;
        rightError = rightAltM - _rightoffsetval;
        leftError = _leftoffsetval - leftAltM;
    ...
    } else if (_profilingval == 1){
        topError = topprofM - _topoffsetval;
        botError = _downoffsetval - downprofM ;
        rightError = rightprofM - _rightoffsetval;
        leftError = _leftoffsetval - leftprofM;

    }

    N_thrust = P_thrust / (P_thrust + R);
    N_roll = P_roll / (P_roll + R);

    if (pilih == 1){
        if (topConfiM > 79 ){//check confidence first
            if (fabs(topError) > CenterThreshold) {
                x_thrust = x_thrust + N_thrust * topError * K * 5;
            }
        }
        //qDebug() <<"active TOP Offset";
    } else if (pilih == 2){
        if (downConfiM > 79 ){
            if (fabs(botError) > CenterThreshold) {
                x_thrust = x_thrust + N_thrust * botError * K * 5;
            }
        }//qDebug() <<"active BOTTOM Offset";
    }else if (pilih == 3 ){
         if (rightConfiM > 79 ){
            if (fabs(rightError) > CenterThreshold) {
                x_roll = x_roll + N_roll * rightError * K * 5;
             }
        }//qDebug() <<"active RIGHT Offset";
    }else if (pilih == 4 ){
        if (leftConfiM > 79 ){
            if (fabs(leftError) > CenterThreshold) {
                x_roll = x_roll + N_roll * leftError * K * 5;
            }
        }//qDebug() <<"active LEFT Offset";

    } else if (pilih == 5){
        if (topConfiM > 79 && rightConfiM > 79){
            //qDebug()<< "Top and Right offset ";
            x_thrust = x_thrust + N_thrust * topError * K *10;
            x_roll = x_roll + N_roll * rightError * K*5;

        }

    } else if (pilih == 6){
        if(downConfiM > 79 && rightConfiM > 79){
            //qDebug()<< "Down and Right offset ";
            x_thrust = x_thrust + N_thrust * botError * K*10;
            x_roll = x_roll + N_roll * rightError * K*5;
        }

    } else if (pilih == 7){
        if (topConfiM>79 && leftConfiM > 79){
            //qDebug()<< "Top and Left offset ";
            x_thrust = x_thrust + N_thrust * topError * K*10;
            x_roll = x_roll + N_roll * leftError * K*5;
        }
    } else if (pilih == 8){
        if(downConfiM >79 && leftConfiM >79){
            //qDebug()<< "Down and Left offset ";
            x_thrust = x_thrust + N_thrust * botError * K*10;
            x_roll = x_roll + N_roll * leftError * K*5;
    ...
    mavlink_msg_manual_control_pack_chan(
        ...
        static_cast<int16_t>(pitch_cruise),
        static_cast<int16_t>(x_roll),
        static_cast<int16_t>(x_thrust),
        static_cast<int16_t>(x_yaw),
        buttons_1,
        0, 0, 0, 0);
    sendMessageOnLinkThreadSafe(sharedLink.get(), message);
...
void Vehicle::centeringkalman(float roll_1, float pitch_1, float yaw_1, float thrust_1, quint16 buttons_1){
    ...
    float leftAlt = leftAltM;
    float leftProf = leftprofM;

    float rightAlt = rightAltM;
    float rightProf = rightprofM;

    float topAlt = topAltM;
    float topProf = topprofM;

    float downAlt = downAltM;
    float downProf = downprofM;
    ...
    float x_thrust = thrust_1; // Initial thrust estimate
    float x_yaw = yaw_1; // Initial thrust estimate
    float pitch_cruise = pitch_1; //Initial estimate for pitch
    ...
    // Calculate reference values for each sensor for Altimeter + Profiling
    float ref_left = leftAlt + leftProf;
    float ref_right = rightAlt + rightProf ;
    float ref_top = topAlt + topProf ;
    float ref_down = downAlt + downProf ;

    // Calculate YAW reference values for each sensor for Altimeter + Profiling
    float refYaw_left = leftProf - leftAlt;
    float refYaw_right = rightProf - rightAlt ;
    ...
    if (_altimeterval == 1 && _profilingval == 1){
        ...
        //qDebug() << "Altimeter and Profiling data fusion";
        if (rightConfiM > 79 && leftConfiM > 79){

            if (_horval == 1){
                error_roll =  ref_right - ref_left;
                if (fabs(error_roll) > CenterThreshold) {
                    // Kalman filter update step
                    // Roll update
                    N_roll = P_roll / (P_roll + R);
                    x_roll = x_roll + N_roll * error_roll * K *5;
                }
        ...
        if (topConfiM > 79 && downConfiM > 79){
            if (_verval == 1){
                // thruttle update
                error_thrust = ref_top - ref_down;
                if (fabs(error_thrust) > CenterThreshold) {
                    N_thrust = P_thrust / (P_thrust + R);
                    x_thrust = x_thrust + N_thrust * error_thrust * K * 5;
                }
        ...
        if (_activeyawval == 1){
            ...
            if (rightConfiM > 79 && leftConfiM > 79){
                error_yaw = refYaw_left - refYaw_right ;
                if (fabs(error_yaw) > CenterThreshold) {
                    N_yaw = P_yaw / (P_yaw + R);
                    x_yaw = x_yaw + N_yaw * error_yaw * K * 5;
            ...
            //qDebug() << "kalmanRoll:"<<x_roll<<", KalmanThrust:"<<x_thrust<<", KalmanYaw:"<<x_yaw;



            if (_activecruiseval == 1){
                if (rightConfiM > 79 && downConfiM > 79 && leftConfiM > 79){
                    pitch_cruise = pitch_cruise + _safetyval * 250;
                    // backward
                    if (_activebackwardval == 1){
                        pitch_cruise = pitch_cruise * -1;
                ...
                //qDebug() << "kalmanRoll:"<<x_roll<<", KalmanThrust:"<<x_thrust<<", KalmanYaw:"<<x_yaw<<", Cruise"<<pitch_cruise;
        ...
        //qDebug() << "kalmanRoll:"<<x_roll<<", KalmanThrust:"<<x_thrust;
        mavlink_msg_manual_control_pack_chan(
            static_cast<uint8_t>(_mavlink->getSystemId()),
            static_cast<uint8_t>(_mavlink->getComponentId()),
            sharedLink->mavlinkChannel(),
        ...
        //qDebug() << "Altimeter only data";

        if (rightConfiM > 79 && leftConfiM > 79){

            // Kalman filter update step
            error_roll =  rightAlt - leftAlt;
            if (_horval == 1){
                // Roll update
                if (fabs(error_roll) > CenterThreshold) {
                    N_roll = P_roll / (P_roll + R);
                ...
                // Roll update and throttle update (bottom) //calib mode
                // buttons_1 = 4; //map joystick number 4 to depth hold mode
                float botError = 0.0f;
                botError = _calibsetval - downAltM ;
                if (fabs(error_roll) > CenterThreshold) {
                    N_roll = P_roll / (P_roll + R);
                    x_roll = x_roll + N_roll * error_roll * K * 5;
                    if (downConfiM > 79){
        ...
        //correction when one sensor have less confidence------------------

        if (rightConfiM > 79 && rightAlt > 1.0 && leftConfiM < 79){
            if (_horval == 1){
                float correctionR = 0.8f;
                float rightError = rightAlt - correctionR;
                if (fabs(rightError) > CenterThreshold) {
                    N_roll = P_roll / (P_roll + R);
                    x_roll = x_roll + N_roll * rightError * K * 5;
                }
        ...
        if (leftConfiM > 79 && leftAlt > 1.0 && rightConfiM < 79){
            if (_horval == 1){
                float correctionL = 0.8f;
                float lefttError = correctionL - leftAlt;
                if (fabs(lefttError) > CenterThreshold) {
                    N_roll = P_roll / (P_roll + R);
                    x_roll = x_roll + N_roll * lefttError * K * 5;
                }
        ...
        // qDebug() << "profiling only data";
        // Kalman filter update step
        // Roll update
        error_roll =  rightProf - leftProf;
        N_roll = P_roll / (P_roll + R);
        x_roll = x_roll + N_roll * error_roll * K;

        // thruttle update
        error_thrust = topProf - downProf;
        N_thrust = P_thrust / (P_thrust + R);
        x_thrust = x_thrust + N_thrust * error_thrust * K;
...
void Vehicle::sendJoystickDataThreadSafe(float roll, float pitch, float yaw, float thrust, quint16 buttons)
{
    ...
    //qDebug() << "RYY Joystick, Roll=" << newRollCommand << ", Pitch=" << newPitchCommand << ", Yaw:" << newYawCommand << ", Throttle:" << newThrustCommand
    //         << ", Buttons:" << buttons ;

/*
    qDebug() << "RYY = Kalman_gain:" << _kalmangval <<", Noise:" << _noiseval << ", Cruise speed:" << _safetyval <<", Active Cruise:"<<_activecruiseval
             <<", Centering Active:"<< _activeval <<", Active Yaw:"<< _activeyawval <<", Altimeter En:"<< _altimeterval << ", Profiling En:"<<_profilingval
             <<", INS En:"<< _insval <<", Active backward:"<<_activebackwardval << ", Active horizontal:" << _horval << ", Active vertical:"<< _verval << ", Center Threshold:" <<_centerthreshold << ", Calibration Mode:" << _activecalibval << ", Altitude(m):"<< _calibsetval;

    qDebug() << "RYY Offset = Top:" << _topoffsetval<<", Active Top:" << _activetopoffsetval << ", Down:" << _downoffsetval
             <<", Active Down"<<_activedownoffsetval<<", Left:"<<_leftoffsetval<<", Active Left:"<<_activeleftoffsetval<<", Right:" << _rightoffsetval << ", Active Right:"<<_activerightoffsetval;
...
*/
    ...
    if (_activetopoffsetval == 1 && _activerightoffsetval == 1){
        offsetkalman(5,newRollCommand,newPitchCommand,newYawCommand,newThrustCommand,buttons);
    } else if (_activedownoffsetval == 1 && _activerightoffsetval == 1){
        offsetkalman(6,newRollCommand,newPitchCommand,newYawCommand,newThrustCommand,buttons);
    } else if (_activetopoffsetval == 1 && _activeleftoffsetval == 1){
        offsetkalman(7,newRollCommand,newPitchCommand,newYawCommand,newThrustCommand,buttons);
    } else if (_activedownoffsetval == 1 && _activeleftoffsetval == 1){
        offsetkalman(8,newRollCommand,newPitchCommand,newYawCommand,newThrustCommand,buttons);
    } else if (_activetopoffsetval == 1){
        offsetkalman(1,newRollCommand,newPitchCommand,newYawCommand,newThrustCommand,buttons);
    } else if (_activedownoffsetval == 1){
        offsetkalman(2,newRollCommand,newPitchCommand,newYawCommand,newThrustCommand,buttons);
    } else if (_activerightoffsetval == 1){
        offsetkalman(3,newRollCommand,newPitchCommand,newYawCommand,newThrustCommand,buttons);
    } else if (_activeleftoffsetval == 1){
        offsetkalman(4,newRollCommand,newPitchCommand,newYawCommand,newThrustCommand,buttons);
    }else if (_activeval == 1){
        centeringkalman(newRollCommand,newPitchCommand,newYawCommand,newThrustCommand,buttons);
        //qDebug() << "centering activated";


    }

    else{
        //qDebug() << "please select correct option";
